...
class BestSolution(object):
    ...
    def update(self, arx, xarchive=None, arf=None, evals=None, real_f=None, segs = None):
        ...
        if isinstance(arx, BestSolution):
            ...
            if arx.f is not None and arx.f < np.inf:
                self.update([arx.x], xarchive, [arx.f], arx.evals, arx.real_f, arx.segs)
            return self
        ...
        if minarf < np.inf and (minarf < self.f or self.f is None):
            ...
            self.x, self.f = arx[minidx], real_f[minidx]
        ...
        self.last.f = real_f[minidx]
        if segs is None:
            self.best_seq = None
        else:
            self.best_seq = segs[minidx]
        # self.last.f = minarf
    def get(self):
        """return ``(x, f, evals)`` """
        return self.x, self.f, self.evals, self.best_seq  # , self.x_geno
