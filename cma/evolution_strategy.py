...
class CMAEvolutionStrategy(interfaces.OOOptimizer):
    ...
    *Selective mirroring* is implemented with option ``CMA_mirrors``
    ...
    def ask_geno(self, number=None, xmean=None, sigma_fac=1):
        ...
        # sigma = sigma if sigma <= 5 * self.sigma0 else self.sigma0
        sigma = 1e-12 if sigma < 1e-12 else sigma
        pop = xmean + sigma * ary
    ...
    def tell_real_seg(self, solutions, function_values, check_points=None,
                      copy=False, real_f=None, segs = None):
        ...
        self.best.update(solutions, self.sent_solutions, function_values, self.countevals, real_f, segs)
    ...
    def tell_real(self, solutions, function_values, check_points=None,
                      copy=False, real_f=None):
        """pass objective function values to prepare for next
        iteration. This core procedure of the CMA-ES algorithm updates
        all state variables, in particular the two evolution paths, the
        distribution mean, the covariance matrix and a step-size.
        ...
        self.countiter += 1  # >= 1 now
        self.countevals += sp.popsize * self.evaluations_per_f_value
        self.best.update(solutions, self.sent_solutions, function_values, self.countevals, real_f, None)
...
def fmin(objective_function, x0, sigma0,
    ...
    where
        ...
        - ``res[-2]`` (``cmaes_simple``) -- class `CMAEvolutionStrategy` instance
